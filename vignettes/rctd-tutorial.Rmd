...
may be specified using the `cell_type_col` parameter in `create.RCTD`.
...
`SpatialExperiment` and `SummarizedExperiment` objects using the `create.RCTD`
...
rctd_config <- create.RCTD(spatial_spe, reference_se, max_cores = 1)
```
...
The `doublet_mode` argument is used to specify the RCTD mode. For the purpose of
demonstration, we'll decompose the data into doublets in this vignette.
