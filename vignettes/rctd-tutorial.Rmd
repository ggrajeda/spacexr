...
object `rctdSim$proportions_spe`. (The ground truth has been stored in the same
format as the RCTD output, which will be discussed later.) Note that this ground
truth will not be known in practice---it is what we will need to estimate with
RCTD.
...
## Creating RctdConfig object

The last step before running RCTD is to create an `RctdConfig` object from our
...
results_spe <- runRctd(rctd_config, rctd_mode = "doublet")
```
...
- Three assays (one in full mode):
    ...
    **NOTE**: Weights are transposed compared to the output of the original
    `dmcable/spacexr` package on GitHub.
