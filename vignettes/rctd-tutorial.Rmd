...
Here, we load counts and cell type annotations from the `simRctd`
...
Next, we will load the spatial transcriptomics data into a `SpatialExperiment`
object with:
...
Here, we load coordinates and counts from the `simRctd` object. Again,
...
ggplot(as.data.frame(spatialCoords(spatial_spe)), aes(x, y)) + 
    geom_point(size = 5) + 
...
`simRctd$proportions_se`. Note that this ground truth will not be known
in practice---it is what we will need to estimate with RCTD.

```{r trueProportions}
unique(assay(simRctd$proportions_se))
```
...
`SpatialExperiment` and `SummarizedExperiment` objects using the `createRctd`
function. Some important configuration options include:
...
We are now ready to run RCTD using the `runRctd` function. 
...
runs. For the purpose of demonstration, we'll decompose the data into doublets
in this vignette.

```{r runRctd, message = FALSE}
results_se <- runRctd(rctd_config, rctd_mode = "doublet")
```
...
`assay(results_se, "weights_full")`. We will visualize these weights later. You
may use `assay(results_se, "weights_unconfident")` to access the less confident
...
in `createRctd` so that these mixtures will be identified as doublets.)
...
plotAllWeights(results_se, r = 0.05, lwd = 0, title = "Doublet Results")
```
<br/>
<br/>
