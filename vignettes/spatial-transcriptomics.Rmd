...
print(dim(counts(reference)))  # Digital Gene Expression matrix dimensions
...
print(dim(counts(puck)))      # Digital Gene Expression matrix dimensions
ggplot(coords(puck), aes(x, y)) +
...
plot_cell_mixture <- function(puck, proportions, title = NULL) {
  ...
  for (i in seq_len(num_cell_types)) {
    ...
    cell_type_data <- data.frame(
      x = coords(puck)$x[is_non_zero] + jitter,
      y = coords(puck)$y[is_non_zero] + jitter,
...
```{r results}
rctd_results <- results(my_rctd)
print(rctd_results$results_df[, c("spot_class", "first_type", "second_type")])
```
...
norm_weights <- normalize_weights(rctd_results$weights)
plot_cell_mixture(
  puck,
  norm_weights,
