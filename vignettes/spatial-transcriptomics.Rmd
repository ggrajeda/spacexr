...
We can view the doublet weights (cell type proportions) for our data. Each row
represents a pixel, and each column represents a cell type. The values indicate
the estimated proportion (from 0 to 1) of each cell type in that pixel:

```{r doubletWeights}
assay(results_se)
```

To see the unconstrained proportions, you can access the full weights using
`assay(results_se, "weights_full")`.

We can also view the classifications of our data:
...
In particular, the data frame above provides us with the following information:

- `spot_class`: A factor variable with the following levels of classification:
    * `singlet` (1 cell type in pixel)
    * `doublet_certain` (2 cell types in pixel)
    * `doublet_uncertain` (2 cell types in pixel, but only confident of 1)
