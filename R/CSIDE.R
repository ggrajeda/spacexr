...
createCside <- function(rctd_results) {
    ...
    new(
        ...
        results = rctd_results
    )
}

#' Runs CSIDE on a \code{\linkS4class{RCTD}} object with a single explanatory
#' variable
#'
#' Identifies cell type specific differential expression (DE) as a function of
#' the explanatory variable. The design matrix contains an intercept column and
#' a column of the explanatory variable. Uses maximum likelihood estimation to
...
#' @param cell_types the cell types used for CSIDE. If null, cell types will be
#'   chosen with aggregate occurrences of at least `cell_type_threshold`, as
#'   aggregated by \code{\link{aggregate_cell_types}}
#' @param cell_type_threshold (default 125) min occurrence of number of cells
#'   for each cell type to be used, as aggregated by
#'   \code{\link{aggregate_cell_types}}
#' @param gene_threshold (default 5e-5) minimum average normalized expression
#'   required for selecting genes
#' @param doublet_mode (default TRUE) if TRUE, uses RCTD doublet mode weights.
#'   Otherwise, uses RCTD full mode weights
#' @param sigma_gene (default TRUE) if TRUE, fits gene specific overdispersion
#'   parameter. If FALSE, overdispersion parameter is same across all genes.
#' @param weight_threshold (default NULL) the threshold of total normalized
#'   weights across all cell types in \code{cell_types} per pixel to be included
#'   in the model. Default 0.99 for doublet_mode or 0.8 for full_mode.
#' @param PRECISION.THRESHOLD (default 0.05) for checking for convergence, the
#'   maximum parameter change per algorithm step
#' @param cell_types_present cell types (a superset of `cell_types`) to be
#'   considered as occurring often enough to consider for gene expression
#'   contamination during the step filtering out marker genes of other cell
#'   types.
#' @param fdr (default 0.01) false discovery rate for hypothesis testing
#' @param test_genes_sig (default TRUE) logical controlling whether genes will
#'   be tested for significance
#' @param normalize_expr (default FALSE) if TRUE, constrains total gene
#'   expression to sum to 1 in each condition.
...
#' @param log_fc_thresh (default 0.4) the natural log fold change cutoff for
...
#'   for each cell type, of significant genes detected by CSIDE. Additionally,
#'   the object contains `internal_vars_de` a list of variables that are used
#'   internally by CSIDE
#' @export
run.CSIDE.single <- function(
    ...
    cell_types_present = NULL, fdr = .01, test_genes_sig = TRUE,
    normalize_expr = FALSE, logs = FALSE, log_fc_thresh = 0.4,
    test_error = FALSE) {
    ...
    run.CSIDE(
        ...
        cell_type_filter = cell_type_filter, log_fc_thresh = log_fc_thresh,
        test_error = test_error
    )
}

#' Runs CSIDE on a \code{\linkS4class{RCTD}} object to detect nonparametric
#' smooth gene expression patterns
#'
#' Identifies cell type specific smooth gene expression patterns. The design
#' matrix contains thin plate spline basis functions spanning the space of
#' smooth functions. Uses maximum likelihood estimation to estimate DE and
...
#'   \code{\linkS4class{SpatialRNA}} object to be used when fitting the model.
#' @param cell_types the cell types used for CSIDE. If null, cell types will be
#'   chosen with aggregate occurences of at least `cell_type_threshold`, as
#'   aggregated by \code{\link{aggregate_cell_types}}
#' @param cell_type_threshold (default 125) min occurence of number of cells for
#'   each cell type to be used, as aggregated by
#'   \code{\link{aggregate_cell_types}}
#' @param gene_threshold (default 5e-5) minimum average normalized expression
#'   required for selecting genes
#' @param doublet_mode (default TRUE) if TRUE, uses RCTD doublet mode weights.
...
#'   maximum parameter change per algorithm step
#' @param cell_types_present cell types (a superset of `cell_types`) to be
#'   considered as occuring often enough to consider for gene expression
#'   contamination during the step filtering out marker genes of other cell
#'   types.
#' @param fdr (default 0.01) false discovery rate for hypothesis testing
#' @param test_error (default FALSE) if TRUE, exits after testing for error
...
run.CSIDE.nonparam <- function(
    ...
    cell_type_threshold = 125, gene_threshold = 5e-5, doublet_mode = TRUE,
    weight_threshold = NULL, sigma_gene = TRUE, PRECISION.THRESHOLD = 0.05,
    cell_types_present = NULL, fdr = .01, test_genes_sig = TRUE, logs = FALSE,
    test_error = FALSE) {
    myRCTD <- createCside(rctd_results)
    ...
    run.CSIDE(
        ...
        fdr = fdr, normalize_expr = FALSE, logs = logs,
        cell_type_filter = cell_type_filter
    )
}

#' Runs CSIDE on a \code{\linkS4class{RCTD}} object for DE across multiple
#' discrete regions
#'
#' Identifies cell type specific differential expression (DE) across multiple
#' discrete regions The design matrix contains for each region a column of 0s
#' and 1s representing membership in that region. Uses maximum likelihood
#' estimation to estimate DE and standard errors for each gene and each cell
...
run.CSIDE.regions <- function(
    ...
    fdr = 0.01, test_genes_sig = TRUE, logs = FALSE, log_fc_thresh = 0.4,
    test_error = FALSE) {
    myRCTD <- createCside(rctd_results)
    ...
    run.CSIDE(
        ...
        test_mode = "categorical", weight_threshold = weight_threshold,
        sigma_gene = sigma_gene, params_to_test = seq_len(dim(X2)[2]),
        PRECISION.THRESHOLD = PRECISION.THRESHOLD,
        test_genes_sig = test_genes_sig,
        cell_types_present = cell_types_present, fdr = fdr,
        normalize_expr = FALSE, logs = logs, log_fc_thresh = log_fc_thresh,
        test_error = test_error
    )
}

#' Runs cell type specific CSIDE on a \code{\linkS4class{RCTD}} object with a
#' general design matrix
#'
#' Identifies cell type specific differential expression (DE) across a general
#' design matrix of covariates. Uses maximum likelihood estimation to estimate
#' DE and standard errors for each gene and each cell type. Selects genes with
...
#'   covariate for explaining differential expression and need to be linearly
#'   independent.
#' @param barcodes the barcodes, or pixel names, of the
#'   \code{\linkS4class{SpatialRNA}} object to be used when fitting the model.
#' @param cell_types the cell types used for CSIDE. If null, cell types will be
#'   chosen with aggregate occurences of at least `cell_type_threshold`, as
#'   aggregated by \code{\link{aggregate_cell_types}}
#' @param cell_type_specific: (default TRUE for all covariates). A logical
#'   vector of length the number of covariates indicating whether each
#'   covariate's DE parameters should be cell type-specific or shared across all
#'   cell types.
#' @param params_to_test: (default 2 for test_mode = 'individual', all
#'   parameters for test_mode = 'categorical'). An integer vector of parameter
...
#'   weights across all cell types in \code{cell_types} per pixel to be included
#'   in the model. Default 0.99 for doublet_mode or 0.8 for full_mode.
#' @param test_mode (default 'individual') if 'individual', tests for DE
#'   individually for each parameter. If 'categorical', then tests for
#'   differences across multiple categorical parameters
#' @param PRECISION.THRESHOLD (default 0.05) for checking for convergence, the
#'   maximum parameter change per algorithm step
#' @param cell_types_present cell types (a superset of `cell_types`) to be
#'   considered as occuring often enough to consider for gene expression
...
#' @param fdr (default 0.01) false discovery rate for hypothesis testing
#' @param normalize_expr (default FALSE) if TRUE, constrains total gene
#'   expression to sum to 1 in each condition. Setting normalize_expr = TRUE is
#'   only valid for testing single parameters with test_mode = 'individual'.
#' @param test_genes_sig (default TRUE) logical controlling whether genes will
#'   be tested for significance
#' @param logs (default FALSE) if TRUE, writes progress to logs/de_logs.txt
#' @param test_error (default FALSE) if TRUE, exits after testing for error
...
run.CSIDE <- function(
    ...
    weight_threshold = NULL, sigma_gene = TRUE, PRECISION.THRESHOLD = 0.05,
    cell_types_present = NULL, test_genes_sig = TRUE, fdr = .01,
    ...
    run.CSIDE.general(
        ...
        cell_type_threshold = cell_type_threshold,
        gene_threshold = gene_threshold, doublet_mode = doublet_mode,
        test_mode = test_mode, weight_threshold = weight_threshold,
        sigma_gene = sigma_gene, PRECISION.THRESHOLD = PRECISION.THRESHOLD,
        params_to_test = params_to_test,
        cell_types_present = cell_types_present,
        test_genes_sig = test_genes_sig, fdr = fdr,
        normalize_expr = normalize_expr, logs = logs,
        cell_type_filter = cell_type_filter, log_fc_thresh = log_fc_thresh,
        test_error = test_error, initialSol = initialSol
    )
}

#' Runs CSIDE on a \code{\linkS4class{RCTD}} object with a general design matrix
#'
#' Identifies differential expression (DE) across a general design matrix of
#' covariates. DE parameters can be cell type-specific or shared across all cell
#' types. Uses maximum likelihood estimation to estimate DE and standard errors
#' for each gene and each cell type. Selects genes with significant nonzero DE.
#' The type of test is determined by \code{test_mode}, and the parameters tested
#' is determined by \code{params_to_test}.
#'
#' @param myRCTD an \code{\linkS4class{RCTD}} object with annotated cell types
#'   e.g. from the \code{\link{run.RCTD}} function.
#' @param X1 a matrix containing the covariates shared across all cell types.
#'   The rownames represent pixel names and should be a subset of the pixels in
#'   the \code{\linkS4class{SpatialRNA}} object. The columns each represent a
#'   covariate for explaining differential expression and need to be linearly
#'   independent.
#' @param X2 a matrix containing the cell type-specific covariates. The rownames
#'   represent pixel names and should be a subset of the pixels in the
#'   \code{\linkS4class{SpatialRNA}} object. The columns each represent a
#'   covariate for explaining differential expression and need to be linearly
#'   independent.
...
#'   parameters for test_mode = 'categorical'). An integer vector of parameter
#'   indices to test. For example c(1,4,5) would test only parameters
#'   corresponding to columns 1, 4, and 5 of the design matrix X2.
#' @param cell_type_threshold (default 125) min occurence of number of cells for
#'   each cell type to be used, as aggregated by
#'   \code{\link{aggregate_cell_types}}
#' @param gene_threshold (default 5e-5) minimum average normalized expression
...
#'   results of the CSIDE algorithm including `gene_fits`, which contains the
#'   results of fits on individual genes, in addition `sig_gene_list`, a list,
#'   for each cell type, of significant genes detected by CSIDE, whereas
...
run.CSIDE.general <- function(
    ...
    weight_threshold = NULL, sigma_gene = TRUE, PRECISION.THRESHOLD = 0.05,
    cell_types_present = NULL, test_genes_sig = TRUE, fdr = .01,
    params_to_test = NULL, normalize_expr = FALSE, logs = FALSE,
    cell_type_filter = NULL, log_fc_thresh = 0.4, test_error = FALSE,
    initialSol = NULL) {
    myRCTD <- createCside(rctd_results)
    ...
    if ((myRCTD@config$doublet_mode != "multi") &&
        ...
        barcodes <- intersect(barcodes, colnames(myRCTD@results))
    }
    cell_types <- choose_cell_types(
    ...
    gene_fits <- get_de_gene_fits(
        X1[barcodes, , drop = FALSE], X2[barcodes, , drop = FALSE], my_beta,
        nUMI[barcodes], gene_list_tot, cell_types,
        restrict_puck(puck, barcodes), barcodes, sigma_init, test_mode,
        numCores = myRCTD@config$max_cores, sigma_gene = sigma_gene,
        initialSol = initialSol, MIN_CHANGE = myRCTD@config$MIN_CHANGE_DE,
        PRECISION.THRESHOLD = PRECISION.THRESHOLD,
        params_to_test = params_to_test, logs = logs
    )
    if (normalize_expr) {
        myRCTD <- normalize_de_estimates(
            myRCTD,
    ...
    if (test_genes_sig) {
        ...
        sig_gene_list <- both_gene_list$sig_gene_list
        all_gene_list <- both_gene_list$all_gene_list
    } else {
        sig_gene_list <- NULL
        all_gene_list <- NULL
    }
    myRCTD@internal_vars_de <- list(
        barcodes = barcodes, cell_types = cell_types,
        doublet_mode = doublet_mode, cell_types_present = cell_types_present,
        my_beta = my_beta, X1 = X1, X2 = X2, test_mode = test_mode,
        params_to_test = params_to_test
    )
    myRCTD@de_results <- list(
        gene_fits = gene_fits, sig_gene_list = sig_gene_list,
...
get_sig_genes <- function(
    puck, myRCTD, gene_list_tot, cell_types, my_beta, barcodes, nUMI, gene_fits,
    cell_types_present, X2, test_mode, params_to_test = 2, fdr = .01,
    p_thresh = 1, log_fc_thresh = 0.4, normalize_expr = FALSE) {
    cti_renorm <- get_norm_ref(
        puck,
        myRCTD@cell_type_info$info[[1]],
        intersect(
            gene_list_tot,
            rownames(myRCTD@cell_type_info$info[[1]])
        ),
    ...
    for (cell_type in cell_types) {
        ...
        if (test_mode == "individual") {
            both_genes <- find_sig_genes_individual(
                cell_type, cell_types, gene_fits, gene_list_type, X2,
            ...
            both_genes <- find_sig_genes_categorical(
                cell_type, cell_types, gene_fits, gene_list_type, X2,
                p_thresh = p_thresh, log_fc_thresh = log_fc_thresh,
                params_to_test = params_to_test
            )
        }
        sig_genes <- both_genes$sig_genes
        all_genes <- both_genes$all_genes
...
find_sig_genes_categorical <- function(
    ...
    if (length(gene_list_type) == 0) {
        stop(
            "find_sig_genes_categorical: cell type ", cell_type, " has not",
            "converged on any genes. Consider removing this cell type from ",
            "the model using the cell_types option."
        )
    }
    if (is.null(params_to_test)) {
        params_to_test <- seq_len(dim(X2)[2])
    }
    n_regions <- length(params_to_test)
    n_cell_types <- length(cell_types)
    cell_ind <- (which(cell_types == cell_type))
    s_mat_ind <- (seq_len(dim(X2)[2])) + (n_regions * (cell_ind - 1))
    p_val_sig_pair <- numeric(length(gene_list_type))
    names(p_val_sig_pair) <- gene_list_type
    log_fc_best_pair <- numeric(length(gene_list_type))
    names(log_fc_best_pair) <- gene_list_type
    sd_vec <- numeric(length(gene_list_type))
    names(sd_vec) <- gene_list_type
    sd_lfc_vec <- numeric(length(gene_list_type))
    names(sd_lfc_vec) <- gene_list_type
    i1_vec <- numeric(length(gene_list_type))
    names(i1_vec) <- gene_list_type
    i2_vec <- numeric(length(gene_list_type))
    names(i2_vec) <- gene_list_type
    for (gene in gene_list_type) {
        con_regions <- get_con_regions(
            gene_fits, gene, dim(X2)[2], cell_ind, n_cell_types
        ...
        p_val_sig_pair[gene] <- min(
            1, ovr_best_p_val * choose(n_regions_con, 2)
        )
    ...
    all_genes <- data.frame(
        sd_lfc_vec[gene_list_type], i1_vec[gene_list_type],
        i2_vec[gene_list_type], sd_vec[gene_list_type],
        p_val_sig_pair[gene_list_type], log_fc_best_pair[gene_list_type]
    )
    rownames(all_genes) <- gene_list_type
    custom_names <- c(
        "sd_lfc", "paramindex1_best", "paramindex2_best", "sd_best",
        "p_val_best", "log_fc_best"
    )
    colnames(all_genes) <- custom_names
    if (length(gene_list_type) > 1) {
        all_genes <- data.frame(
            all_genes,
            gene_fits$all_vals[rownames(all_genes), params_to_test, cell_ind],
        ...
        colnames(all_genes)[(length(custom_names) + 1):length(all_genes)] <-
            c(
                lapply(
                    params_to_test,
                    function(x) paste0("mean_", x)
                ),
                lapply(
                    params_to_test,
                    function(x) paste0("sd_", x)
                )
            )
    } else {
        if (length(gene_list_type) == 1) {
            all_genes <- data.frame(t(unlist((c(
                all_genes,
                gene_fits$all_vals[
                    rownames(all_genes), params_to_test, cell_ind
                ],
                gene_fits$s_mat[rownames(all_genes), s_mat_ind[params_to_test]]
            )))))
            rownames(all_genes) <- gene_list_type
            colnames(all_genes)[
                (length(custom_names) + 1):length(all_genes)
            ] <- c(
                lapply(params_to_test, function(x) paste0("mean_", x)),
                lapply(params_to_test, function(x) paste0("sd_", x))
            )
        } else {
            all_genes <- list()
        }
...
find_sig_genes_individual <- function(
    ...
    if (length(gene_list_type) == 0) {
        stop(
            "find_sig_genes_individual: cell type ", cell_type, " has not ",
            "converged on any genes. Consider removing this cell type from ",
            "the model using the cell_types option."
        )
    }
    ...
    all_genes <- data.frame(
        z_score[gene_list_type], log_fc[gene_list_type], s_vec[gene_list_type],
        best_ind[gene_list_type]
    )
...
get_de_gene_fits <- function(
    X1, X2, my_beta, nUMI, gene_list, cell_types, puck, barcodes, sigma_init,
    test_mode, numCores = 4, sigma_gene = TRUE, MIN_CHANGE = 0.001,
    PRECISION.THRESHOLD = 0.05, params_to_test = 2, initialSol = NULL,
    logs = FALSE) {
    results_list <- fit_de_genes(
        X1, X2, my_beta, nUMI, gene_list, puck, barcodes, sigma_init, test_mode,
        numCores = numCores, sigma_gene = sigma_gene, initialSol = initialSol,
    ...
    rownames(mean_val) <- gene_list
    colnames(mean_val) <- cell_types
    rownames(intercept_val) <- gene_list
    colnames(intercept_val) <- cell_types
    d_vals <- matrix(0, nrow = N_genes, ncol = dim(X2)[2] * length(cell_types))
    s_mat <- matrix(0, nrow = N_genes, ncol = dim(X2)[2] * length(cell_types))
    precision_mat <- matrix(
        0,
        nrow = N_genes,
        ncol = dim(X2)[2] * length(cell_types)
    )
    con_all <- matrix(
        FALSE,
        nrow = N_genes,
    ...
    rownames(precision_mat) <- gene_list
    rownames(con_all) <- gene_list
    rownames(s_mat) <- gene_list
    rownames(con_mat) <- gene_list
    rownames(error_mat) <- gene_list
    ...
    list(
        mean_val = mean_val, con_val = con_val, ll_val = ll_val, I_val = I_val,
        s_mat = s_mat, n.iter = n_val, d_vals = d_vals,
        intercept_val = intercept_val, all_vals = all_vals,
        precision_mat = precision_mat, sigma_g = sigma_g, con_mat = con_mat,
        con_all = con_all, error_mat = error_mat
    )
}

fit_de_genes <- function(
    X1, X2, my_beta, nUMI, gene_list, puck, barcodes, sigma_init, test_mode,
    numCores = 4, MIN_CHANGE = 1e-3, sigma_gene = TRUE,
    PRECISION.THRESHOLD = 0.05, initialSol = NULL, logs = FALSE) {
    results_list <- list()
    if (numCores == 1) {
        for (i in seq_along(gene_list)) {
            ...
            if (!is.null(initialSol)) {
                alpha2_init <- t(as.matrix(initialSol[i, ]))
            }
            results_list[[i]] <- estimate_gene_wrapper(
                Y, X1, X2, my_beta, nUMI, sigma_init, test_mode,
                verbose = FALSE, n.iter = 200, MIN_CHANGE = MIN_CHANGE,
        ...
        BiocParallel::bplapply(seq_along(gene_list), function(i) {
            if (logs) {
                if (i %% 1 == 0) { ## 10
                    log_message <- paste0(
                        "Testing sample: ", i, " gene ", gene_list[i], "\n"
                    )
                    write(log_message, file = out_file, append = TRUE)
                }
            }
            gene <- gene_list[i]
            Y <- counts(puck)[gene, barcodes]
            ...
            estimate_gene_wrapper(
                Y, X1, X2, my_beta, nUMI, sigma_init, test_mode,
                verbose = FALSE, n.iter = 200, MIN_CHANGE = MIN_CHANGE,
                sigma_gene = sigma_gene, alpha2_init = alpha2
            )
        })
    }
    return(results_list)
