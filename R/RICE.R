...
#' @param gene_cutoff_reg minimum normalized gene expression for genes to be
#'   included.
...
#' @param class_df (optional) if not NULL, then a dataframe mapping each cell
#'   type to a cell class, so that RCTD will report confidence on the class
#'   level.
...
createRctdNoRef <- function(
    ...
    if (length(gene_list) == 0) {
        stop(
            "create.RCTD.unsupervised: 0 regression differentially expressed ",
            "genes found"
        )
    }
...
runRice <- function(
    ...
    RCTD@cell_type_info$renorm <- list(
        RCTD@cell_type_info$info[[1]][
            RCTD@internal_vars$gene_list_reg, cell_types
        ],
        cell_types,
        length(cell_types)
    )
