normalize_de_estimates <- function(
    myRCTD, normalize_expr, remove_junk = TRUE, param_position = 2) {
    if (remove_junk) {
        all_genes <- rownames(myRCTD@de_results$gene_fits$con_mat)
        junk_genes <- all_genes[c(
            grep("MT-", all_genes), grep("RPS", all_genes),
            grep("RPL", all_genes)
        )]
    ...
    for (cell_type in myRCTD@internal_vars_de$cell_types) {
        con_genes_all <- names(
            which(myRCTD@de_results$gene_fits$con_mat[, cell_type])
        )
        con_genes <- con_genes_all[
            !(tolower(con_genes_all) %in% tolower(junk_genes))
        ]
        if (!("mean_val_cor" %in% names(myRCTD@de_results$gene_fits))) {
            myRCTD@de_results$gene_fits$mean_val_cor <- list()
        }
        if (normalize_expr) {
            reg_1 <- myRCTD@de_results$gene_fits$all_vals[
                con_genes_all, 1, cell_type
            ]
            reg_2 <- (
                reg_1 +
            ...
            myRCTD@de_results$gene_fits$mean_val_cor[[cell_type]] <- (
                mean_val_cor
            )
        } else {
            myRCTD@de_results$gene_fits$mean_val_cor[[cell_type]] <- (
                myRCTD@de_results$gene_fits$mean_val[, cell_type]
            )
        }
    }
    return(myRCTD)
