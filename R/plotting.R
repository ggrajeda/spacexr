...
plot_doublets <- function(puck, doublets, resultsdir, cell_type_names) {
  ...
  plot <- ggplot2::ggplot(my_table, ggplot2::aes(x = x, y = y)) +
    ggplot2::geom_point(ggplot2::aes(size = .15, shape = 19, color = class)) +
...
plot_doublets_type <- function(puck, doublets_base, resultsdir, cell_type_names) {
  ...
  for (cell_type in cell_type_names) {
    ...
    if (length(barcodes) > 0) {
      ...
      plots[[i]] <- ggplot2::ggplot(my_table, ggplot2::aes(x = x, y = y)) +
        ggplot2::geom_point(ggplot2::aes(size = .15, shape = 19, color = class)) +
        ggplot2::scale_color_manual(values = my_pal[pres]) +
        ggplot2::scale_shape_identity() +
        ggplot2::theme_bw() +
...
plot_weights_doublet <- function(cell_type_names, puck, resultsdir, weights_doublet, results_df) {
  ...
  for (i in 1:length(cell_type_names)) {
    ...
    all_weights_vec <- as.vector(all_weights)
    names(all_weights_vec) <- rownames(all_weights)
...
plot_doub_occur_stack <- function(doub_occur, resultsdir, cell_type_names) {
  ...
  colnames(doub_occur) <- cell_type_names
  ...
  plot <- ggplot2::ggplot(data, ggplot2::aes(fill = second_type, y = count, x = first_type)) +
    ggplot2::geom_bar(position = "stack", stat = "identity") +
    ggplot2::scale_fill_manual(values = my_pal[pres]) +
...
plot_class <- function(puck, barcodes_cur, my_class, counter_barcodes = NULL, title = NULL) {
  ...
  plot <- ggplot2::ggplot(my_table, ggplot2::aes(x = x, y = y)) +
    ggplot2::geom_point(ggplot2::aes(size = 0.5, shape = 19, color = class)) +
    ggplot2::scale_shape_identity() +
