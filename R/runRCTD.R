test_single_beads <- function(puck, gene_list, cell_type_info, trust_model = FALSE, constrain = TRUE, OLS = FALSE) {
  ...
  cell_type_lev <- factor(seq_len(n_cell_types))
  cell_type_map <- data.frame(cindex = seq_len(n_cell_types), row.names = cell_type_names)
  if (trust_model) {
  ...
  rownames(conf_mat$table) <- cell_type_names
  colnames(conf_mat$table) <- cell_type_names
