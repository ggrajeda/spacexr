...
solveIRWLS.effects_trust <- function(Y, X1, X2, my_beta, test_mode, verbose = FALSE,
    ...
    converged_vec <- check_converged_vec(
        X1, X2, my_beta, itera, n.iter,
...
estimate_gene_wrapper <- function(Y, X1, X2, my_beta, nUMI, sigma_init, test_mode, verbose = F, n.iter = 200, MIN_CHANGE = 1e-3, sigma_gene = T, PRECISION.THRESHOLD = 0.05, alpha2_init = NULL) {
    if (sigma_gene) {
