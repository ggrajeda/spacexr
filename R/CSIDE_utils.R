filter_genes <- function(puck, threshold = 5e-5, batch_size = 1000) {
  ...
  gene_means <- numeric(length(rownames(puck@counts)))
  names(gene_means) <- rownames(puck@counts)
...
get_gene_list_type_wrapper <- function(myRCTD, cell_type, cell_types_present) {
  ...
  cti_renorm <- get_norm_ref(
    myRCTD@originalSpatialRNA, myRCTD@cell_type_info$info[[1]],
    gene_list, myRCTD@internal_vars$proportions
  )
  return(get_gene_list_type(
    myRCTD@internal_vars_de$my_beta, myRCTD@internal_vars_de$barcodes, cell_type,
...
choose_cell_types <- function(myRCTD, barcodes, doublet_mode, cell_type_threshold, cell_types,
  ...
  if (passed_cell_types) {
    ...
    message(paste0(
      "choose_cell_types: detected the following cell types occuring at least cell_type_threshold times: ",
  ...
  while (TRUE) {
    ...
    if (passed_cell_types) {
      stop(paste0(
        "choose_cell_types: cannot include cell types: ", diff_types,
      ...
      warning(paste0(
        "choose_cell_types: removing cell types: ", diff_types,
        " because these cell types did not contain sufficient pixels passing total cell type weight of weight_threshold = ", thresh,
        ". Consider removing this cell type or lowering weight_threshold"
      ))
    }
