...
get_de_pop <- function(
    cell_type, de_results_list, cell_prop, params_to_test, use.groups = FALSE,
    ...
    if (!use.groups) {
        ...
        colnames(de_pop) <- c(
            "tau", "log_fc_est", "sd_est", "Z_est", "p_cross", "delta",
...
one_ct_genes <- function(
    ...
    if (filter) {
        gene_final <- intersect(
            gene_big[which(q_vals < q_thresh & p_vals < p_thresh)],
    ...
    gene_df <- cbind(
        de_pop[gene_big, ], cell_prop[gene_big, c(cell_type)],
